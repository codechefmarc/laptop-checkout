...
<form method="POST" action="{{ route('taxonomy.pool.store') }}">
  @csrf
    ...
    <div class="pb-12">
      <div class="mt-10 flex flex-col gap-y-2">
        ...
        <div>
          ...
          <div class="mt-2">
          <textarea name="description" id="description" rows="3" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 w-full">{{ old('description', '') }}</textarea>
            @error('description')
              <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
            @enderror
          </div>

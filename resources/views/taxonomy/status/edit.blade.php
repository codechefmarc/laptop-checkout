...
<form method="POST" action="{{ route('taxonomy.status.update', $status->id) }}">
  @csrf
  @method('PATCH')

  <input type="hidden" name="return_url" value="{{ $returnUrl }}">
  <input type="hidden" name="tailwind_class" value="{{ $status->tailwind_class }}">
    ...
    <div class="pb-12">
      <div class="mt-10 flex flex-col gap-y-2">
        ...
        <div>
          ...
          <input type="number" id="weight" class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 w-48" name="weight" value="{{ $status->weight }}">
        </div>
